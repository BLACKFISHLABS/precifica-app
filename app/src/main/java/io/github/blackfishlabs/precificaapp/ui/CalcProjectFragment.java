...
public class CalcProjectFragment extends BaseFragment {

    public static final String TAG = CalcProjectFragment.class.getName();
    ...
    protected int provideContentViewResource() {
        return R.layout.fragment_calc_project;
    }

    public static CalcProjectFragment newInstance() {
        return new CalcProjectFragment();
    }
    ...
    private void calculate() {
        int totalHour = (valuePerHour.getWorkHour() * valuePerHour.getWeekDays());

        if (totalHour > 0) {
            BigDecimal total = valuePerHour.getMoneyFocus().multiply(BigDecimal.valueOf(totalHour));
